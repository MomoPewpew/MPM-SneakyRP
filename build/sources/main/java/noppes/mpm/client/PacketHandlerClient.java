    ...
    if (type == EnumPackets.PING) {
      ...
      if (animation == 0) {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          player.worldObj.spawnParticle(EnumParticleTypes.VILLAGER_ANGRY, x, player.posY + 0.800000011920929D + (player.getRNG().nextFloat() * player.height / 2.0F) - player.getYOffset() - data.getBodyY(), z, d0, d1, d2, new int[0]);
        }
      }
