...
public class CommandProp extends MpmCommandInterface {
	...
	public void execute(MinecraftServer server, ICommandSender icommandsender, String[] args) throws CommandException {
		...
		Float propOffsetX = (args.length > 2) ? Float.valueOf(args[5]) : 1.0F;
		Float propOffsetY = (args.length > 3) ? Float.valueOf(args[6]) : 1.0F;
		Float propOffsetZ = (args.length > 4) ? Float.valueOf(args[7]) : 1.0F;
		Float propScaleX = (args.length > 5) ? Float.valueOf(args[2]) : 1.0F;
		Float propScaleY = (args.length > 6) ? Float.valueOf(args[3]) : 1.0F;
		Float propScaleZ = (args.length > 7) ? Float.valueOf(args[4]) : 1.0F;
		...
		data.newProp(propItemStack, bodyPartName, propOffsetX, propOffsetY, propOffsetZ, propScaleX, propScaleY, propScaleZ, propRotateX, propRotateY, propRotateZ);
	}
